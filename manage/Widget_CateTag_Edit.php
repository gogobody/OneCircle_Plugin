...
class Widget_CateTag_Edit extends Widget_Abstract_Metas
{
    ...
    public function form($action = NULL)
    {
        ...
        $name = new Typecho_Widget_Helper_Form_Element_Text('name', NULL, NULL,
            _t('分类的标签名称 *'), _t('这是标签在站点中显示的名称.可以使用中文,如 "地球".'));
        $form->addInput($name);

        /** 标签缩略名 */
        $slug = new Typecho_Widget_Helper_Form_Element_Text('slug', NULL, NULL,
            _t('标签缩略名'), _t('建议使用英文，标签缩略名用于创建友好的链接形式, 如果留空则默认使用标签名称.'));
        $form->addInput($slug);
...
//        $this->security->protect();
        if ($this->request->is('do=insert') or $this->request->is('do=update') or $this->request->is('do=delete')
        or $this->request->is('do=merge') or $this->request->is('do=refresh')){
            $this->security->protect();
...
//            $this->on($this->request->is('do=merge'))->mergeTag(); // api有点问题
