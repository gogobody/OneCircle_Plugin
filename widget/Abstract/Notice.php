...
class Widget_Abstract_Notices extends Widget_Abstract{
    ...
    public function filter(array $value){
        ...
        if($value['type']=='comment' || $value['type']=='at'){
            $comment = $this->widget('Widget_Comments_Archive@parentId_'.$value['srcId'],array('parentId'=>$value['srcId']));
    ...
    public function select(){
        return $this->db->select()->from('table.notices');
    }
    ...
    public function insert(array $rows){
        return $this->db->query($this->db->insert('table.notices')->rows($rows));
    }
    /**
     * 更新记录方法
     *
    ...
    {
        return $this->db->query($condition->update('table.notices')->rows($rows));
    }
    ...
    public function size(Typecho_Db_Query $condition)
    {
        return $this->db->fetchObject($condition
            ->select(array('COUNT(DISTINCT table.notices.id)' => 'num'))
            ->from('table.notices')
